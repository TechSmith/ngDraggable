   ...
   .directive('ngDrop', ['$parse', '$timeout', '$window', 'ngDraggable', function ($parse, $timeout, $window, ngDraggable) {
      return {
         ...
         link: function (scope, element, attrs) {
            ...
            var onDragOverCallback = $parse(attrs.ngDragOver);
            var onDragOutCallback = $parse(attrs.ngDragOut);

            var elementDraggedOver = false;
            ...
            var onDragMove = function(evt, obj) {
               if(! _dropEnabled)return;
               var touching = isTouching(obj.x,obj.y,obj.element);
               if(touching) {
                  if(!elementDraggedOver) {
                     elementDraggedOver = true;
                     $timeout(function(){
                        onDragOverCallback(scope, {$data: {dropData: _data, dragData:  obj.data}});
                     });
                  }
               } else if(elementDraggedOver) {
                  elementDraggedOver = false;
                  $timeout(function(){
                     onDragOutCallback(scope, {$data: {dropData: _data, dragData:  obj.data}});
                  });
               }
               $timeout(function(){
